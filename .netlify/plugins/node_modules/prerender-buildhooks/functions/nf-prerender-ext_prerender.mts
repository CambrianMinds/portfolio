...
var MEMORY_SIZES = ["1gb", "2gb", "4gb"];
var DEFAULT_MEMORY_SIZE = "1gb";
var prerenderConfigSchema = z.object({
  enabled: z.boolean().default(false),
  skipAgentsWithJS: z.boolean().default(true),
  memorySize: z.enum(MEMORY_SIZES).default(DEFAULT_MEMORY_SIZE),
  ...
  useBrowserCache: z.boolean().default(true)
});
var optionalPrerenderConfigSchema = prerenderConfigSchema.omit({ enabled: true, skipAgentsWithJS: true, memorySize: true });
...
async function createPrerenderConfig(req) {
  ...
  const config2 = {
    ...
    // Don't allow browser cache when allowing multiple domains, to avoid any cache attacks
    useBrowserCache: !allowRemoteHosts && optionalConfig.useBrowserCache,
...
var PASSTHROUGH_SECURITY_HEADERS = [
  "x-frame-options",
...
var prerender_default = async (req, context) => {
  ...
  try {
    ...
    await page.setCacheEnabled(config2.useBrowserCache);
    ...
    if (originalStatus !== 200 && originalStatus !== 304)
...
function buildSuccessResponse(originalResponse, responseBody, config2, needsJsonEncoding) {
  ...
  if (!config2.allowRemoteHosts) {
    for (const header of PASSTHROUGH_SECURITY_HEADERS) {
...
function logSuccess(targetUrl, timer, stats, completionReason, htmlSize) {
  ...
  console.log(
    `SUCCESS: ${targetUrl} | ${timer.getTotalElapsed()}ms total (${phaseTimings}) | 200 status | ${htmlSize}B HTML | ${stats.skippedRequests} skipped, ${stats.completedRequests} completed, ${stats.cachedRequests} cached, ${stats.failedRequests} failed | completed via ${completionReason}`
  );
}
...
function getMemoryConfig() {
  const rawMemorySize = process.env.NETLIFY_PRERENDER_MEMORY_SIZE?.trim();
  if (rawMemorySize && rawMemorySize !== DEFAULT_MEMORY_SIZE && MEMORY_SIZES.includes(rawMemorySize)) {
    return { memory: rawMemorySize };
  }
  return {};
}
var config = {
...
if (config.memory) {
  console.log(
    "Memory set to:",
    config.memory,
    process.env.AWS_LAMBDA_FUNCTION_MEMORY_SIZE ? `, actual: ${process.env.AWS_LAMBDA_FUNCTION_MEMORY_SIZE}` : ""
  );
}
export {
  config,
